    ...
    const bool followsDiscontinuity) noexcept
{
    ...
    for (std::size_t frame = 0; frame < frameCount; ++frame) {
        processSample(channels_[0], left != nullptr ? left[frame] : 0.0F);
        if (channelCount == 2)
            processSample(channels_[1], right != nullptr ? right[frame] : 0.0F);
    }
...
void PeakRmsBallistics::processSample(ChannelState& state, const float sample) const noexcept
{
    const auto magnitude = std::isfinite(sample) ? std::abs(static_cast<double>(sample)) : 0.0;
